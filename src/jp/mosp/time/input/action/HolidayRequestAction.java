		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (grantDate != null) {
				...
				if (getSystemDate().compareTo((Date)grantDate) < 0) {
					// 設定
					aryLblStyle[i] = TimeConst.STYLE_GRAY;
				}
			}
			// 項目名取得
			...
			aryLblPaidLeaveRemainDays[i] = getNumberOfDayAndHour(remainDaysDouble, remainHoursInt);
			aryLblPaidLeaveGrantDays[i] = getNumberOfDayAndHour(grantDaysDouble, grantHoursInt);
