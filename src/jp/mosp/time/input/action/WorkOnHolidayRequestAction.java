...
//		vo.setPltEditSubstitute1Range(String.valueOf(TimeConst.CODE_WORK_ON_HOLIDAY_SUBSTITUTE_ON));
	...
	protected void draft() throws MospException {
		...
		boolean substituteFlag = Integer.toString(TimeConst.CODE_WORK_ON_HOLIDAY_SUBSTITUTE_ON).equals(
				vo.getPltEditSubstitute())
				|| Integer.toString(TimeConst.CODE_WORK_ON_HOLIDAY_SUBSTITUTE_ON_WORK_TYPE_CHANGE).equals(
						vo.getPltEditSubstitute());
		// 振替休日DTOの準備
	...
	*/
	...
	protected void setActivationDate() throws MospException {
		...
		if (vo.getModeActivateDate().equals(PlatformConst.MODE_ACTIVATE_DATE_CHANGING)) {
			...
			if (Integer.toString(TimeConst.CODE_WORK_ON_HOLIDAY_SUBSTITUTE_ON).equals(vo.getPltEditSubstitute())) {
				...
				// 午前又は午後の場合
				if (editWorkRange != null
						&& !editWorkRange.isEmpty()
						&& (Integer.toString(TimeConst.CODE_WORK_ON_HOLIDAY_SUBSTITUTE_AM).equals(editWorkRange) || Integer
							.toString(TimeConst.CODE_WORK_ON_HOLIDAY_SUBSTITUTE_PM).equals(editWorkRange))) {
					// 振替申請区分設定
	...
	protected void setWorkOnHolidayDtoFields(WorkOnHolidayRequestDtoInterface dto) throws MospException {
		...
		dto.setWorkTypeCode("");
		// 振替出勤(勤務形態変更あり)の場合
		if (Integer.toString(TimeConst.CODE_WORK_ON_HOLIDAY_SUBSTITUTE_ON_WORK_TYPE_CHANGE).equals(
				vo.getPltEditSubstitute())) {
			// 勤務形態コード設定
	...
	protected void setSubstituteDtoFields(SubstituteDtoInterface dto) throws MospException {
		...
		if (getInt(vo.getPltEditSubstitute()) == TimeConst.CODE_WORK_ON_HOLIDAY_SUBSTITUTE_ON
	...
	protected String getSubstituteName(int code) {
		...
		// 振替・午前・午後の場合
		if (code == TimeConst.CODE_WORK_ON_HOLIDAY_SUBSTITUTE_ON
				|| code == TimeConst.CODE_WORK_ON_HOLIDAY_SUBSTITUTE_AM
