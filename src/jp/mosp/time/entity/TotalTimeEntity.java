...
public class TotalTimeEntity implements TotalTimeEntityInterface {
	...
	protected void totalHoliday() {
		...
		specialHolidayMap = new HashMap<String, Float>();
		// その他休暇回数群(キー：休暇コード)を準備
		otherHolidayMap = new HashMap<String, Float>();
		// 欠勤回数群(キー：休暇コード)を準備
		absenceMap = new HashMap<String, Float>();
		// 休暇種別毎に処理(休暇回数群を初期化)
		for (HolidayDtoInterface dto : holidaySet) {
			...
			// 休暇回数群に初期値を設定
			addHolidayTimes(holidayCode, holidayType, 0F);
		}
		// 休暇申請毎に処理(回数を加算)
		for (HolidayRequestDtoInterface dto : holidayRequestList) {
			// 休暇コードと休暇区分と休暇範囲を取得
			...
			if (holidayRange == TimeConst.CODE_HOLIDAY_RANGE_ALL) {
				// 回数は休暇取得日数
				times = DateUtility.getDayDifference(dto.getRequestStartDate(), dto.getRequestEndDate()) + 1;
			}
			// 前半休か後半休の場合
			if (holidayRange == TimeConst.CODE_HOLIDAY_RANGE_AM || holidayRange == TimeConst.CODE_HOLIDAY_RANGE_PM) {
			...
			if (times <= 1) {
				...
				addHolidayTimes(holidayCode, holidayType, times);
				continue;
			}
			...
			for (Date targetDate : holidayDateList) {
				...
				// 法定休日出勤又は所定休日出勤である場合
				if (TimeUtility.isWorkOnLegalHoliday(workTypeCode)
		...
		// 特別休暇回を取得
		totalSpecialHoliday = totalHoliday(specialHolidayMap);
		// その他休暇回数を取得
		totalOtherHoliday = totalHoliday(otherHolidayMap);
		// 欠勤回数を取得
		totalAbsence = totalHoliday(absenceMap);
	}
	...
	protected void addHolidayTimes(String holidayCode, int holidayType, float times) {
		...
		// 合計回数を取得
		Float total = holidayMap.get(holidayCode);
		// 合計回数が取得できない場合
		if (total == null) {
			// 合計回数を初期化
			total = 0F;
		...
		// 加算して休暇回数群に設定
		holidayMap.put(holidayCode, total + times);
	}
	
	/**
	 * 休暇回数を集計する。<br>
	 * <br>
	 * 休暇区分毎に作成された休暇回数群(キー：休暇コード)の回数を合計する。<br>
	 * <br>
	 * @param map 休暇回数群(キー：休暇コード)
	 * @return 休暇回数
	 */
	...
	protected boolean isSubHolidayExpired(SubHolidayDtoInterface dto) {
		...
		// 勤怠設定情報を取得
		TimeSettingDtoInterface timeSettingDto = timeSettingMap.get(targetDate);
	...
	protected RequestEntity getRequestEntity(Date targetDate) {
		...
		// 申請エンティティを再取得
		entity = new RequestEntity(personalId, targetDate);
		// 申請エンティティ群に設定
		...
		entity.setOverTimeRequestList(TotalTimeUtility.getOvertimeRequestList(overtimeRequestList, workflowMap,
				targetDate));
		entity.setWorkTypeChangeRequestDto(TotalTimeUtility.getWorkTypeChangeRequestList(workTypeChangeRequestList,
				workflowMap, targetDate));
	...
	public Map<String, Float> getSpecialHolidayMap() {
		return specialHolidayMap;
	}
	
	@Override
	public Map<String, Float> getOtherHolidayMap() {
		return otherHolidayMap;
	}
