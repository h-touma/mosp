...
public class WorkTypeCardAction extends TimeSettingAction {
	...
	protected void checkAutoBeforeOverWork() throws MospException {
		...
		// 勤務前残業自動申請
		if (vo.getPltAutoBeforeOverWork().equals(String.valueOf(MospConst.INACTIVATE_FLAG_ON))) {
			// 無効の場合、チェックなし
			return;
		}
		// 時短時間1取得 
		...
		// 時短時間1有給確認
		if (vo.getPltShort1Type().equals(String.valueOf(MospConst.INACTIVATE_FLAG_OFF))) {
			// 有給の場合、チェックなし
			return;
		}
