...
//		checkWorkOnHolidayOverlap(dto);
	...
	public void checkCancel(WorkOnHolidayRequestDtoInterface dto) throws MospException {
		...
		if (attendanceDto != null) {
			...
			// 承認可能又は1次戻又は承認済の場合
			if (workflowDto != null
					&& (workflowIntegrate.isApprovable(workflowDto) || WorkflowUtility.isFirstReverted(workflowDto) || WorkflowUtility
						.isCompleted(workflowDto))) {
				// エラーメッセージ追加
				mospParams.addErrorMessage(TimeMessageConst.MSG_ATTENDANCE_CANCEL_CHECK, getNameSubstituteDay()
						+ mospParams.getName("Of", "WorkManage", "Application"), mospParams.getName("Release"));
				return;
			}
		}
		...
		if (substituteDto != null) {
			...
			if (WorkflowUtility.isWithDrawn(workflowDto) || WorkflowUtility.isDraft(workflowDto)
					|| WorkflowUtility.isReverted(workflowDto) || WorkflowUtility.isFirstReverted(workflowDto)) {
				return;
			}
			...
			// 取下・下書・差戻・一次戻の場合
			if (WorkflowUtility.isWithDrawn(workflowDto) || WorkflowUtility.isDraft(workflowDto)
					|| WorkflowUtility.isReverted(workflowDto)) {
				// 処理なし
				return;
			}
