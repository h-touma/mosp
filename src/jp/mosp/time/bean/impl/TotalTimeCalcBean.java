...
public class TotalTimeCalcBean extends TimeBean implements TotalTimeCalcBeanInterface {
	...
	public void initBean() throws MospException {
		...
		totalTransRegist = (TotalTimeTransactionRegistBeanInterface)createBean(TotalTimeTransactionRegistBeanInterface.class);
		employeeTransRegist = (TotalTimeEmployeeTransactionRegistBeanInterface)createBean(TotalTimeEmployeeTransactionRegistBeanInterface.class);
		...
		// 勤怠集計エンティティ取得クラス
		totalTimeEntityRefer = (TotalTimeEntityReferenceBeanInterface)createBean(TotalTimeEntityReferenceBeanInterface.class);
		// プラットフォームマスタ参照クラスを取得
	...
	@Override
	public TotalTimeDataDtoInterface calc(String personalId, int targetYear, int targetMonth) throws MospException {
		// 勤怠集計エンティティを取得
		TotalTimeEntityInterface entity = getTotalTimeEntity(personalId, targetYear, targetMonth);
		// 各種申請及び振替休日情報から承認済でないものを除去
		removeUnusedRequests(entity, true);
	...
	protected void registVacation(TotalTimeEntityInterface entity) throws MospException {
		...
		Map<String, Float> specialHolidayMap = entity.getSpecialHolidayMap();
		// 休暇コード毎に処理
		for (String holidayCode : specialHolidayMap.keySet()) {
			// 特別休暇集計情報を準備
			...
			dto.setTimes(specialHolidayMap.get(holidayCode));
			// 登録
		...
		Map<String, Float> otherHolidayMap = entity.getOtherHolidayMap();
		// 休暇コード毎に処理
		for (String holidayCode : otherHolidayMap.keySet()) {
			// 特別休暇集計情報を準備
			...
			dto.setCalculationMonth(targetMonth);
			dto.setHolidayCode(holidayCode);
			dto.setTimes(otherHolidayMap.get(holidayCode));
			// 登録
		...
		// 欠勤回数群(キー：休暇コード)を取得
		Map<String, Float> absenceMap = entity.getAbsenceMap();
		// 休暇コード毎に処理
		for (String holidayCode : absenceMap.keySet()) {
			// 特別休暇集計情報を準備
			...
			dto.setTimes(absenceMap.get(holidayCode));
			// 登録
	...
	protected boolean isWorkflowNeeded(WorkflowDtoInterface dto, boolean isCompleted) {
		...
		// 申請済であるかを確認
		return WorkflowUtility.isApplied(dto);
	}
	...
	protected void round(TotalTimeDataDtoInterface dto, TimeSettingDtoInterface setDto) {
		...
		// 休憩時間
		dto.setRestTime(getRoundMinute(dto.getRestTime(), setDto.getRoundMonthlyRest(),
				setDto.getRoundMonthlyRestUnit()));
		// 深夜休憩時間
		dto.setRestLateNight(getRoundMinute(dto.getRestLateNight(), setDto.getRoundMonthlyRest(),
				setDto.getRoundMonthlyRestUnit()));
		// 所定休出休憩時間
		...
		// 合計遅刻時間
		dto.setLateTime(getRoundMinute(dto.getLateTime(), setDto.getRoundMonthlyLate(),
				setDto.getRoundMonthlyLateUnit()));
		// 遅刻30分以上時間
