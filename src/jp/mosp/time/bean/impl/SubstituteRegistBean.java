...
public class SubstituteRegistBean extends PlatformBean implements SubstituteRegistBeanInterface {
	...
	protected void checkHalfRequest(SubstituteDtoInterface dto) throws MospException {
		...
		if (!beforeList.isEmpty() && dto.getHolidayRange() == TimeConst.CODE_HOLIDAY_RANGE_AM || !beforeList.isEmpty()
				&& dto.getHolidayRange() == TimeConst.CODE_HOLIDAY_RANGE_PM) {
			// 申請ユーティリティ取得
			RequestUtilBeanInterface requestUtil2 = (RequestUtilBeanInterface)createBean(RequestUtilBeanInterface.class);
			// 対象日準備
			...
			// 勤務日形態コードが所定休日の場合
			if (!workTypeCode1.equals(TimeConst.CODE_HOLIDAY_PRESCRIBED_HOLIDAY)) {
				// 同じ勤務形態でない場合
	...
	public void checkHolidayDate(SubstituteDtoInterface dto) {
		...
		if (TimeConst.CODE_HOLIDAY_LEGAL_HOLIDAY.equals(dto.getSubstituteType())) {
			// 法定休日の場合
			mospParams.addErrorMessage(TimeMessageConst.MSG_REQUEST_CHECK_6,
					TimeNamingUtility.legalHoliday(mospParams), mospParams.getName("HalfDay", "Transfer"),
					getNameSubstituteDay());
		}
	}
	...
	protected void checkEntered(SubstituteDtoInterface dto) throws MospException {
		if (entranceReference.isEntered(dto.getPersonalId(), dto.getSubstituteDate())) {
			// 入社している場合
			return;
		}
		// 入社していない場合
	...
	protected void addSubstituteDateErrorMessage(Date date) {
		mospParams.addErrorMessage(TimeMessageConst.MSG_REQUEST_CHECK_3, new String[]{ DateUtility.getStringDate(date),
			getNameGoingWorkDay(), getNameSubstituteDay() });
	}
	...
	protected void addOthersRequestErrorMessage(Date date, String requestTitle) {
		mospParams.addErrorMessage(TimeMessageConst.MSG_REQUEST_CHECK_12, new String[]{ getStringDate(date),
			requestTitle });
	}
