	...
	protected int paidLeaveByTheHourImport(List<ImportFieldDtoInterface> fieldList, String[] data) throws MospException {
		...
		if (startTime == null) {
			...
			addRequiredErrorMessage(
					mospParams.getProperties().getCodeItemName(TimeFileConst.CODE_IMPORT_TYPE_TMD_HOLIDAY_REQUEST,
							TmdHolidayRequestDao.COL_START_TIME), null);
			return 0;
		}
		...
		if (holidayRequestRegist.isLegalDaysOff(workTypeCode) || holidayRequestRegist.isPrescribedDaysOff(workTypeCode)
				|| holidayRequestRegist.isWorkOnLegalDaysOff(workTypeCode)
				|| holidayRequestRegist.isWorkOnPrescribedDaysOff(workTypeCode)) {
			// 法定休日・所定休日・法定休日労働・所定休日労働の場合
			return 0;
